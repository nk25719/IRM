...
DATASETS: tuple[DatasetDefinition, ...] = (
    ...
    DatasetDefinition(
        ...
        "Customer Contacts",
        "CRM",
        "Outlook email-history contacts staged for CRM enrichment. Organization matches require review.",
        REGISTRY_VERSION,
        REGISTRY_UPDATED_AT,
        (
            f("Name", "Name", required=True, description="Display name from Outlook-derived email history."),
            f("Email", "Email", required=True, data_type="email", description="Email address used as the dedupe key."),
            f("Company / Domain", "Company / Domain", description="Workbook company/domain grouping; email-derived domain wins if they disagree."),
            f("Emails Exchanged", "Emails Exchanged", data_type="number", description="Approximate email activity count, not distinct conversations."),
            f("Role", "Role", description="Usually blank in this source; do not invent roles."),
            f("Phone", "Phone", description="Best-effort extracted phone text requiring verification."),
            f("Notes", "Notes"),
        ),
        example_rows=(
            {"Name": "Jane Example", "Email": "jane@examplehospital.com", "Company / Domain": "examplehospital.com", "Emails Exchanged": 12, "Phone": "+961 1 000 000"},
            {"Name": "Purchasing", "Email": "purchasing@examplehospital.com", "Company / Domain": "examplehospital.com", "Emails Exchanged": 35},
        ),
        import_supported=True,
        export_supported=True,
        export_table="contacts",
        export_order_by="email_domain",
        template_filename="customer_contacts_import_template.xlsx",
    ),
    DatasetDefinition(
        "manufacturers",
        "Manufacturers",
